...
void piDD(string line,Data &e) {
  ...
  do {
    if (ParseExpression(line,val)) {
      for (int i=0;i!=4;++i) { e.push((ubyte)val); val>>=8; }
...
void piDT(string line,Data &e) {
  ...
  do {
    if (ParseExpression(line,val)) {
      check24(val); for (int i=0;i!=3;++i) { e.push((ubyte)val); val>>=8; }
    } else { error("Syntax error"); return; }
  } while (comma(line));
  checkjunk(line);
}
...
void piDW(string line,Data &e) {
  ...
  do {
    if (ParseExpression(line,val)) {
      check16(val); e.push((ubyte)val); e.push((ubyte)(val>>8));
    } else { error("Syntax error"); return; }
  } while (comma(line));
  checkjunk(line);
}
...
void piASCMAP(string line, Data &) {
  ...
  if (need(line,"=>")) {
    ...
    for (int i=first; i!=last; ++i) { check8(b); map[i]=(ubyte)b; b+=a; }
  } else {
    int oadres=adres;
...
void piASCMAPRESET(string line, Data &) {
  for (int i=0; i!=256; ++i) map[i]=(ubyte)i;
  again=1; checkjunk(line);
}
...
void initPidata() {
  ...
  for (int i=0; i!=256; ++i) smap[i]=map[i]=(ubyte)i;
}
