  ...
  for (vector<_Member*>::iterator im=_memtab.begin(); im!=_memtab.end(); ++im) {
    ...
    if (d.empty()) {
      ...
      case 1:
        ...
        val=check8(val); e.push((ubyte)val);
        break;
      case 2:
        if (!ParseExpression(d,val)) error("Syntax error"); checkjunk(d);
        val=check16(val); e.push((ubyte)val); e.push((ubyte)(val>>8));
        break;
      case 3:
        if (!ParseExpression(d,val)) error("Syntax error"); checkjunk(d);
        val=check24(val); for (int i=0;i!=3;++i) { e.push((ubyte)val); val>>=8; }
        break;
      case 4:
        if (!ParseExpression(d,val)) error("Syntax error"); checkjunk(d);
        /*val=check32(val);*/ for (int i=0;i!=4;++i) { e.push((ubyte)val); val>>=8; }
        break;
      case 5:
        f.clear();
        getbytes(d,f);
